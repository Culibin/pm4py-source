...
def construct_place_aware(pn1, im1, fm1, pn2, im2, fm2, skip, marking_vector_pt, marking_vektor_trace):
    """
    Constructs the synchronous product net of two given Petri nets.
    ...
    sync_net = petri.petrinet.PetriNet('synchronous_product_net of %s and %s' % (pn1.name, pn2.name))
    t1_map, p1_map, new_marking_vector_pt = __copy_into_place_aware(pn1, sync_net, True, skip, marking_vector_pt)
    t2_map, p2_map, new_marking_vector_trace = __copy_into_place_aware(pn2, sync_net, False, skip, marking_vektor_trace)
    ...
    return sync_net, sync_im, sync_fm, new_marking_vector_pt, new_marking_vector_trace
...
def __copy_into_place_aware(source_net, target_net, upper, skip, marking_vector):
    t_map = {}
    p_map = {}
    for t in source_net.transitions:
        name = (t.name, skip) if upper else (skip, t.name)
